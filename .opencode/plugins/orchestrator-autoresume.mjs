...
const CHILD_RESUME_CONFIG = {
  ...
  explorer: {
    ...
    prompt:
      "If you have genuinely new searches to run, run them and append only the new findings. If your prior response already covered the bounded scope, do not restate it — return <status>blocked</status> with a one-line reason (e.g. 'scope already covered' or 'no usable starting point'). Never repeat content from your prior response.",
    shouldResume: (child) => hasIncompleteChildTodos(child) && child.hasTerminalStatus !== true,
    skipNoWorkReason: "explorer already returned terminal status or no incomplete child todos",
  },
  designer: {
