...
describe('shouldReleaseAutoFollowFromScroll', () => {
    ...
    test('does not release for near-bottom scroll clamping caused by content height changes', () => {
        expect(shouldReleaseAutoFollowFromScroll({
            state: 'following',
            previousScrollTop: 1500,
            nextScrollTop: 1460,
            previousScrollHeight: 2100,
            nextScrollHeight: 2060,
            isNearBottomAfterScroll: true,
            isProgrammatic: false,
            hasExplicitUserIntent: true,
        })).toBe(false);
    });

    test('still releases for explicit upward user scroll near the bottom when content height is stable', () => {
        expect(shouldReleaseAutoFollowFromScroll({
            state: 'following',
            previousScrollTop: 1500,
            nextScrollTop: 1460,
