...
export async function createSessionRecord(
  ...
  directoryOverride?: string | null,
  parentID?: string | null,
  options?: { isGitGenerationSession?: boolean },
): Promise<Session | null> {
  try {
    lastCreateSessionError = null
...
export async function archiveSessions(sessionIds: string[]): Promise<{ archivedIds: string[]; failedIds: string[] }> {
  ...
  if (successfulIds.length > 0) {
    const successfulSnapshots = filterSnapshotsById(sessionSnapshots, successfulIds)
    const archivedSnapshotIds = new Set(successfulSnapshots.map((session) => session.id))
    const missingSnapshotIds = successfulIds.filter((sessionId) => !archivedSnapshotIds.has(sessionId))
    useGlobalSessionsStore.getState().archiveSessionSnapshots(successfulSnapshots, archivedAt)
    if (missingSnapshotIds.length > 0) {
      useGlobalSessionsStore.getState().archiveSessions(missingSnapshotIds, archivedAt)
    }
...
export async function unarchiveSessions(sessionIds: string[]): Promise<{ unarchivedIds: string[]; failedIds: string[] }> {
  ...
  if (successfulIds.length > 0) {
    useGlobalSessionsStore.getState().unarchiveSessions(successfulIds)
    for (const sessionId of successfulIds) {
