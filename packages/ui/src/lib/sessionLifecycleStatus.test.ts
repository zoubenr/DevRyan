...
describe("deriveSessionLifecycleStatus", () => {
  ...
  test("plan-proposed is suppressed while assistant is active (shows streaming instead)", () => {
    // This is the 'plan-mode message that produced clarifying questions
    // instead of a plan' case: planIndicatorState may be stale but the
    // assistant is doing work — show that work, not a stale proposed flag.
    const status = deriveSessionLifecycleStatus(base({
      planIndicatorState: 'proposed',
      assistantActivity: 'streaming',
      planModeOn: true,
    }))
    expect(status).toEqual({ kind: 'streaming', planModeOn: true })
  })
