...
- `packages/web/server/lib/opencode/version-policy.js`: Target external OpenCode runtime policy. DevRyan recommends `anomalyco/opencode` v1.15.10 and surfaces the upstream install command while still using the user/system `opencode` binary.
...
- `buildVisibleSkillPolicy({ skills, hiddenSkills })`: builds the allowed skill names/directories used to sanitize managed packaged-agent permissions.
- `sanitizeAgentSkillPolicy(frontmatter, policy)`: rewrites an agent frontmatter permission block so `permission.skill` defaults to deny; skill-capable agents without an explicit wildcard deny get all visible skills, while explicitly restricted agents keep only their previous visible allows.
