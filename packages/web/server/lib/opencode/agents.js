...
function listStaleAgentModelOverrides(workingDirectory, options = {}) {
  const knownNames = new Set(getBaseConfigAgents(workingDirectory).map((agent) => agent.name));
...
function getBaseConfigAgents(workingDirectory) {
...
function assertKnownAgentName(agentName, workingDirectory) {
...
function listConfigAgents(workingDirectory, options = {}) {
  ...
  return getBaseConfigAgents(workingDirectory)
    .map((agent) => applyAgentModelOverride(agent, overrides[agent.name]))
